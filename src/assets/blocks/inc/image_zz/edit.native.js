...
export class ImageEdit extends React.Component {
    ...
    render() {
        ...
        const { align, url, width, alt, href, id, linkTarget, sizeSlug } = attributes;

        const sizeOptions = map(imageSizes, ({ name, slug }) => ({
            value: slug,
            name
        }));
        const sizeOptionsValid = find(sizeOptions, ["value", DEFAULT_SIZE_SLUG]);
...
export default compose([
    withSelect((select, props) => {
        ...
        const shouldGetMedia = id && isSelected && getProtocol(url) !== "file:";
        return {
